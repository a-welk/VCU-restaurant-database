...
Sort the items from Subway that are vegan in ascending order by price.
...
This query retrieves affordable menu items (less than $12) containing "chicken" in their names, displaying their names, descriptions, prices, and the respective restaurant names, sorted by price from least to most expensive.
```{python}
#| echo: false
#| eval: true
sql = f"""
...
This query will retrieve all restaurants that are west of the VCU compass.
```{python}
#| echo: false
#| eval: true
sql = f"""
...
This query will retrieve all restaurants that are located on West Grace street.
```{python}
#| echo: false
#| eval: true
sql = f"""
...
This query will retrieve a list of the restaurants that are literally on the VCU campus.
```{python}
#| echo: false
#| eval: true
sql = f"""
...
This query will retrieve all the restaurants that offer vegan options and display the number of vegan dishes per restaurant.
```{python}
#| echo: false
#| eval: true
sql = f"""
...
This query will retrieve the number of dishes from all the restaurants that are Halal.
