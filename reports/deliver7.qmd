...
- **Final Video Presentation**: [Link to Design Video](https://vcu.mediaspace.kaltura.com/media/Alex+Welk%27s+Zoom+Meeting/1_4f8mto6f)
...
In terms of security and privacy measures, we prioritized safeguarding sensitive information like database access credentials. To achieve this, we utilized a .env file, which served as a secure repository for crucial details like usernames, passwords, hosts, and database names. These vital configurations, such as 'user,' 'password,' 'host,' and 'database,' were stored within this file. The .env was stored in a safe environment. 
...
AND item_dietary_preference_id = 20 -- Assuming 'Vegetarian' has a dietary_preference_id of 20
...
14.This query retrieves number of restaurants by formality level
```{python}
sql = f"""
...
### Accessing the Database using these endpoints

- /api/tables (GET) - Lists tables in the database.
- /api/restaurants (GET, POST) - Gets all restaurants or creates a new one.
- /api/restaurants/'ID' (GET) - Fetches restaurant details by ID.
- /api/reviews/'name' (GET) - Retrieves reviews for a restaurant.
- /api/locations (GET) - Gets all location details.
- /api/accessibility (GET) - Fetches accessibility details.
- /api/locations/accessibility/name/'name' (GET) - Retrieves accessibility details and restaurant location by name.
- /api/menu/ID/'ID' (GET) - Gets menu items by restaurant ID.
- /api/menu/restaurant/'name' (GET) - Retrieves menu items by restaurant name.
- /api/item (GET) - Fetches item details.
- /api/menu_items (GET) - Retrieves menu items.
- /api/dietary_preference (GET) - Gets dietary preferences.
- /api/dietary_preference/<preference> (GET) - Retrieves menu items based on a specific dietary preference.
- /api/restaurants_reviews (GET) - Retrieves reviews along with associated restaurant details.
- /api/bus_stops_near_vc (GET) - Fetches restaurants, their locations, and nearby bus stops near VCU.
...
- ***Environment Setup:**
  ...
  - Use `pip` to install required packages: `flask`, `pandas`, `pymysql`, `sqlalchemy`, and `python-dotenv`.

- ***Environment Variables:**
  - Create a `.env` file in your project directory.
  - Add sensitive information (like database credentials) in the `.env` file.

- ***Running the Flask App:**
  - Execute the Flask application scriptusing `python project-flask.py`.

- ***Accessing the API:**
  - Once the app is running, access the API endpoints by visiting `http://127.0.0.1:5000` in your web browser. We had to run it to successfully render this qmd.

1.Print the first 10 restaurants: Sends a GET request to retrieve restaurant data from the */api/restaurants* endpoint and prints the names of the first 10 restaurants received in the response.
```{python}
import requests

url = 'http://127.0.0.1:5000/api/restaurants'

# Make a GET request to retrieve restaurant data
...
if response.status_code == 200:
    ...
    # Print the first five restaurants
...
2.Shows specific restaurant info: Sends a GET request to fetch details of a restaurant with a specific ID (in this case, ID 29). Prints the retrieved restaurant's name, formality level, average total per person, and restaurant type.
```{python}
import requests

url = 'http://127.0.0.1:5000/api/restaurants/29'  # Endpoint with ID 25

# Make a GET request to retrieve restaurant data with ID 25
response = requests.get(url)

# Check if the request was successful (status code 200)
if response.status_code == 200:
    # Get the JSON data for the restaurant(s) with ID 25
    restaurants = response.json()

    # If the response is a list of restaurants
    if isinstance(restaurants, list):
        # Iterate through the list of restaurants
        for restaurant in restaurants:
            print(f"Restaurant Name: {restaurant['restaurant_name']}")
            print(f"Formality Level: {restaurant['restaurant_formality_level']}")
            print(f"Avg Total per Person: {restaurant['restaurant_avg_total_per_person']}")
            print(f"Restaurant Type: {restaurant['restaurant_type']}")
            # Add other details as needed
    else:
        # Access details for a single restaurant
        print(f"Restaurant Name: {restaurants['restaurant_name']}")
        print(f"Formality Level: {restaurants['restaurant_formality_level']}")
        print(f"Avg Total per Person: {restaurants['restaurant_avg_total_per_person']}")
        print(f"Restaurant Type: {restaurants['restaurant_type']}")
        # Add other details as needed
else:
    print(f"Request failed with status code: {response.status_code}")
...
3.Retrieve Spoon's Reviews: Sends a GET request to fetch reviews for a restaurant named "Spoon" from the */api/reviews/Spoon* endpoint. Parses the retrieved data and prints details such as review ID, rating, and text.
```{python}
import requests
import json

url = 'http://127.0.0.1:5000/api/reviews/Spoon'
...
if response.status_code == 200:
    ...
    # Assuming 'review_id', 'review_rating', and 'review_text' are nested dictionaries with keys like 'i'
    i = 0  # Starting index
    while str(i) in parsed_data['review_id']:
        review_id = parsed_data['review_id'][str(i)]
        review_rating = parsed_data['review_rating'][str(i)]
        review_text = parsed_data['review_text'][str(i)]
...
4.Retrieves items from every menu that are halal: Sends a GET request to fetch menu items that match the dietary preference "halal" from the */api/dietary_preference/halal* endpoint. Prints details including restaurant name, item ID, item name, description, and price for all retrieved items.
```{python}
import requests
import json

url = 'http://127.0.0.1:5000/api/dietary_preference/halal'

# Make a GET request to retrieve item data
response = requests.get(url)
...
if response.status_code == 200:
    ...
    # Assuming 'item_id', 'item_name', 'item_description', 'item_price', 'restaurant_name' are present in the data
    i = 0  # Starting index
    while str(i) in data['item_id']:
        item_id = data['item_id'][str(i)]
        item_name = data['item_name'][str(i)]
        item_description = data['item_description'][str(i)]
        item_price = data['item_price'][str(i)]
        restaurant_name = data['restaurant_name'][str(i)]
...
5.Retrieves the location of a specific restaurant and extra accessibility info: Sends a GET request to fetch location and accessibility details for a restaurant named "Pupatella" from the */api/locations/accessibility/name/Pupatella* endpoint. Prints information including location, accessibility ID, miles to VCU, nearby bus stops, and accessibility notes.
```{python}
import requests
import json

url = 'http://127.0.0.1:5000/api/locations/accessibility/name/Pupatella'
...
if response.status_code == 200:
    ...
    # Assuming 'item_id', 'item_name', 'item_description', 'item_price', 'restaurant_name' are present in the data
    i = 0  # Starting index
    while str(i) in data['accessibility_id']:
        location_address = data['location_address'][str(i)]
        accessibility_id = data['accessibility_id'][str(i)]
        accessibility_restaurant_id = data['accessibility_restaurant_id'][str(i)]
        accessibility_miles_to_VCU = data['accessibility_miles_to_VCU'][str(i)]
        accessibility_nearby_bus_stops = data['accessibility_nearby_bus_stops'][str(i)]
        accessibility_notes = data['accessibility_notes'][str(i)]

        # Display the extracted details
        print(f"Location of Restaurant: {location_address}")
        print(f"Accessibility_id: {accessibility_id}")
        print(f"Accessibility_restaurant_id: {accessibility_restaurant_id}")
        print(f"Miles to VCU: {accessibility_miles_to_VCU}")
        print(f"Nearby Bus Stops: {accessibility_nearby_bus_stops}")
...
6.Shows every item from the menu of a restaurant with ID 12: Sends a GET request to retrieve all menu items for a restaurant with ID 12 from the */api/menu/ID/12* endpoint. Prints details such as item ID, item name, description, and price for each item.
```{python}
import requests
import json

url = 'http://127.0.0.1:5000/api/menu/ID/12'
...
# Source Code Links for DDL, DML ,and Flask API
...
- [Flask API Code](https://github.com/cmsc-vcu/cmsc508-fa2023-prj-restaurantdirectory-group21/blob/main/src/project-flask.py)

# Evaluation

## Future Considerations

If we had more time, we'd delve deeper into fortifying our system with multiple layers of security to safeguard valuable information. This involves implementing stricter measures like user authentication and authorization, ensuring only authorized individuals access sensitive API endpoints. We'd also aim to expand the API's functionality by incorporating additional HTTP methods like PUT and DELETE. These methods empower users to modify or remove existing data, offering a more comprehensive set of actions within the API. It's a step toward creating a more robust and versatile platform, but it requires careful planning and resources to implement securely.

## Reflections on the Project

Throughout this project, we delved into setting up a Flask API, integrating it with a MySQL database, and crafting complex SQL queries. We learned the ins and outs of handling various HTTP methods, but encountered challenges implementing certain functionalities like PUT and DELETE in our API, recognizing the need for more practice. Collaboration played a pivotal role in our project's success. As a team, we synchronized our skills, had well time management and tackled challenges together. 
